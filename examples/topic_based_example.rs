...
async fn main() -> Result<()> {
    ...
    }
    {
        ...
        println!("CNBC available topics (first 5): {:?}\n", &cnbc_topics[..5.min(cnbc_topics.len())]);
    }
        ...
        match wsj.opinions().await {
            Ok(articles) => {
                ...
                if let Some(first) = articles.first() {
                    println!("  First article: {}", first.title.as_ref().unwrap_or(&"No title".to_string()));
                }
            }
    ...
    // Fetch WSJ world news using topic name
    {
        ...
        match wsj.fetch_topic("RSSWorldNews").await {
            Ok(articles) => {
                println!("✓ WSJ World News (via fetch_topic): {} articles", articles.len());
                if let Some(first) = articles.first() {
                    println!("  First article: {}", first.title.as_ref().unwrap_or(&"No title".to_string()));
                }
            }
    ...
    // Fetch CNBC technology news using topic name
    {
        ...
        match cnbc.fetch_topic("technology").await {
            Ok(articles) => {
                println!("✓ CNBC Technology (via fetch_topic): {} articles", articles.len());
                if let Some(first) = articles.first() {
                    println!("  First article: {}", first.title.as_ref().unwrap_or(&"No title".to_string()));
                }
            }
