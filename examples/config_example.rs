...
async fn main() -> Result<()> {
    ...
    println!(
        ...
        default_client.config().timeout_seconds
    );
    println!("Default user agent: {}", default_client.config().user_agent);
    println!(
        "Default max retries: {}",
        default_client.config().max_retries
    );
    ...
    println!(
        "Custom retry delay: {} ms",
        custom_client.config().retry_delay_ms
    );
    ...
    match wsj.opinions().await {
        Ok(articles) => {
            println!(
                "Successfully fetched {} WSJ opinion articles with custom config",
                articles.len()
            );
